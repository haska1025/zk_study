...
char g_service[512] ={ 'd','t','s','0' };
const char *g_path ="/NameService";

//watch function when child list changed
void zktest_watcher_g(zhandle_t* zh, int type, int state, const char* path, void* watcherCtx);
//show all process ip:pid
void zktest_show_list(zhandle_t *zkhandle, const char *path);
//if success, the g_mode will become MODE_MONITOR
void zktest_choose_mater(zhandle_t *zkhandle, const char *path);
//get localhost ip:pid
void zktest_getlocalhost(char *ip_pid, int len);

void zktest_print_usage();
void zktest_get_option(int argc, const char* argv[]);
...
void zktest_watcher_g(zhandle_t* zh, int type, int state, const char* path, void* watcherCtx)
{
    ...
    if(type == ZOO_CHILD_EVENT &&
        ...
        zktest_show_list(zh, child_path);
...
void zktest_show_list(zhandle_t *zkhandle, const char *path)
{
    ...
    zktest_getlocalhost(localhost, sizeof(localhost));
...
int zktest_create(zhandle_t *zkhandle, const char *path, const char *value, int flag)
{
    int ret=0;
  ...
  //  if(ret != ZOK){
        ret = zoo_create(zkhandle,
                ...
                bufferlen);
        if(ret != ZOK){
            printf("failed to create the path(%s) ret(%d)\n", path, ret);
        }else{
...
int main(int argc, const char *argv[])
{
    ...
    zktest_get_option(argc, argv);

    zhandle_t* zkhandle = zookeeper_init(g_host, 
            zktest_watcher_g, 
            timeout, 
            0, 
            NULL, 
    ...
    if (zkhandle == NULL)
    {
        fprintf(stderr, "Error when connecting to zookeeper servers. errno(%d)\n", errno);
        exit(EXIT_FAILURE);
    }
 
    printf("first node path(%s)\n", g_path);
    zktest_create(zkhandle, g_path, "NameServiceTest", 0);

    sprintf(path_buffer, "%s/%s", g_path, g_service);
    zktest_create(zkhandle, path_buffer, "NameService Test", 0);
    ...
    if(g_mode == PROVIDER_MODE){
        ...
        sprintf(child_path, "%s/%s/provider", g_path, g_service);
        zktest_show_list(zkhandle, child_path);
